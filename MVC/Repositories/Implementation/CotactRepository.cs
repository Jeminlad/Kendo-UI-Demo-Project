...
namespace KendoDemo.Repositories.Implementation;
...
public class CotactRepository : IContactInterface
{
    ...
    public void Delete(t_Contact data)
    {
        throw new NotImplementedException();
    }
    ...
    public t_Contact GetOne(int id)
    {
        throw new NotImplementedException();
    }

    public void Insert(t_Contact data)
    {
        throw new NotImplementedException();
    }
    ...
    public async Task<List<vm_Contact>> GetAllWithUserName()
    {
        DataTable dt = new DataTable();
        NpgsqlCommand cm = new NpgsqlCommand("select t_contact.*,t_user.c_username from t_contact inner join t_user on t_contact.c_userid = t_user.c_userid ", _conn);
        _conn.Close();
        _conn.Open();
        NpgsqlDataReader datar = cm.ExecuteReader();
        if (datar.HasRows)
        ...
        List<vm_Contact> contactList = new List<vm_Contact>();
        contactList = (from DataRow dr in dt.Rows
                       select new vm_Contact()
                       {
                           c_ContactId = Convert.ToInt32(dr["c_contactid"]),
                           c_UserId = int.Parse(dr["c_userid"].ToString()),
